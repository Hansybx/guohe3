...
def get_school_calendar(username, password):
    ...
    semester = ''
    if now_week < 33:
        semester = str(now_year - 1) + "-" + str(now_year) + "-2"
    else:
        semester = str(now_year)+"-"+str(now_year+1)+"-1"

    start_day, end_day = time_get(username, password,semester)
...
def time_get(username, password,semester):
    reg = r'<font color="red">请先登录系统</font>'
    session = login(username, password)
    response = session.get('http://jwgl.just.edu.cn:8080/jsxsd/jxzl/jxzl_query?Ves632DSdyV=NEW_XSD_WDZM&xnxq01id='+semester)
...
if __name__ == '__main__':
    a = get_school_calendar('182210711114', 'hanzy2000')
    print(a)
